...
description: |
  Maps and documents SystemVerilog codebases by orchestrating parallel subagents.
  Creates .gateflow/map/ with hierarchy, signals, FSMs, clock domains, types, and more.
  Use when user says "map this codebase", "cartographer", "document architecture",
  "understand this project", or when onboarding to a new RTL codebase.
  Automatically detects if map exists and updates only changed sections.
---

# SV Cartographer

Maps SystemVerilog codebases of any size using parallel Sonnet subagents.
...
3. Plan subagent assignments (group files, handle large files)
4. Spawn Sonnet subagents in parallel (ALL in single message)
5. Synthesize subagent reports into `.gateflow/map/` files
...
> Auto-generated by GateFlow Cartographer
